...
import { useLozad } from '@/directives/lozad';
import { markdownToHTML, MarkdownRenderOption } from '@/utils/markdown'
...
const markdownHTML = computed<string>(() => {
  if (!props.markdown) { return props.html || '' }
  return markdownToHTML(props.markdown, { ...props.renderOptions })
})
</script>

<template>
  <section ref="element" v-html="markdownHTML"
    :class="[plain ? 'global-markdown-plain' : 'global-markdown-html', { compact }]"></section>
</template>
